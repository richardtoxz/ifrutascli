...
function listarProdutos() {
  ...
  produtos.forEach(prod => {
    ...
    console.log(
      `ID: ${prod.id}, Nome: ${prod.nome}, Preço: R$${prod.preco.toFixed(2)}, Quantidade: ${prod.quantidade}, ` +
...
async function buscarProduto() {
  ...
  if (resultado.length > 0) {
    resultado.forEach(prod => {
      const categoria = categorias.find(c => c.id === prod.categoriaId);
      console.log(
        `ID: ${prod.id}, Nome: ${prod.nome}, Preço: R$${prod.preco.toFixed(2)}, Quantidade: ${prod.quantidade}, Categoria: ${categoria ? categoria.nome : 'Categoria não encontrada'}`
      );
    });
  } else {
